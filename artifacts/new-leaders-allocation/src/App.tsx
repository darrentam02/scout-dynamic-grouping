...
function SelectionCriteria() {
  ...
  const [rank1, rank2] = DEFAULT_KPI_CONFIG.rankPoints;
  return <section className="card-surface rounded-2xl border border-border" data-testid="panel-selection-criteria">
    <details open className="group">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 md:p-6"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-muted-foreground">How groups are chosen</p><h2 className="mt-2 text-xl font-bold tracking-[-.04em]">Selection criteria</h2></div><ChevronRight size={18} className="text-muted-foreground transition-transform group-open:rotate-90" /></summary>
      <div className="border-t border-border px-5 pb-6 pt-5 md:px-6">
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">Preference points</p><p className="mt-2 font-mono-ui text-lg font-medium text-primary">1st +{rank1}</p><p className="font-mono-ui text-lg font-medium text-primary">2nd +{rank2}</p><p className="font-mono-ui text-lg font-medium text-muted-foreground">3rd +0</p></div>
          <div className="rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">Safety locks</p><ul className="mt-2 space-y-1 text-xs leading-5 text-muted-foreground"><li><strong className="text-primary">P1/P2</strong> need {safetySkills.p1p2.join(" / ")}</li><li><strong className="text-primary">P5/P6</strong> need {safetySkills.p5p6.join(" / ")}</li></ul></div>
          <div className="rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">Soft KPI</p><p className="mt-2 font-mono-ui text-sm leading-6 text-muted-foreground">rank1 ×0.4 + rank2 ×0.2 + parity ×0.2 + coverage ×0.1 + contribution ×0.1</p></div>
        </div>
        <div className="mt-5"><p className="mb-2 text-xs font-bold text-muted-foreground">Skill fit — per-destination weight (higher = better fit for that pair of patrols)</p><div className="overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[420px] text-left"><thead><tr className="border-b border-border bg-secondary/60 text-[10px] uppercase tracking-[.12em] text-muted-foreground"><th className="px-3 py-2 font-bold">Skill</th>{clusterLabels.map((label) => <th key={label} className="px-3 py-2 text-right font-bold">{label}</th>)}</tr></thead><tbody>{SKILL_NAMES.map((skill) => { const [a, b, c] = WEIGHT_MATRIX[skill]; return <tr key={skill} className="border-b border-border/60 last:border-0"><td className="px-3 py-1.5 text-xs font-semibold text-foreground">{skill}</td><td className="px-3 py-1.5 text-right font-mono-ui text-xs text-muted-foreground">{a}</td><td className="px-3 py-1.5 text-right font-mono-ui text-xs text-muted-foreground">{b}</td><td className="px-3 py-1.5 text-right font-mono-ui text-xs text-muted-foreground">{c}</td></tr>; })}</tbody></table></div></div>
        <p className="mt-5 text-xs leading-5 text-muted-foreground"><strong className="text-primary">How a run works:</strong> Monte Carlo shuffles the roster 500 times; every run places leaders by preference points + skill fit, enforces safety locks and capacity (6 per patrol), then splits each pair of patrols for gender balance with skill-driven tie-breaks. The best-scoring run (soft KPI) is kept.</p>
      </div>
    </details>
  </section>;
}
...
function AllocationSummary({ room }: { room: Room }) {
  ...
  const pct = (value: number) => `${Math.round(value * 100)}%`;
  const mc = room.mcSummary;
  const softRows: [string, number][] = mc ? [
    ["1st choice hit rate", mc.best.kpi.rank1HitRate],
    ["2nd choice hit rate", mc.best.kpi.rank2HitRate],
    ["3rd / forced rate", mc.best.kpi.rank3OrForcedRate],
    ["Gender parity score", mc.best.kpi.genderParityScore],
    ["Skill coverage", mc.best.kpi.coverageScore],
    ["Contribution fit", mc.best.kpi.contributionScore],
    ["Weighted KPI", mc.best.kpi.weightedTotal],
  ] : [];
  return <section data-testid="panel-allocation-summary"><div className="mb-4"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-muted-foreground">Soft-coded KPI</p><h2 className="mt-2 text-xl font-bold tracking-[-.05em] text-primary">Allocation summary</h2></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="card-surface rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">1st choice hit rate</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.rank1}%</p></div><div className="card-surface rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">2nd choice hit rate</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.rank2}%</p></div><div className="card-surface rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">3rd choice / Forced</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.rank3}% <span className="text-sm text-muted-foreground">({summary.forced} forced)</span></p></div><div className="card-surface rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">Gender parity</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.genderParity}%</p></div><div className="card-surface rounded-xl border border-border p-4"><p className="text-xs font-bold text-muted-foreground">Skill coverage</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.coverage}%</p></div><div className="rounded-xl border border-primary/25 bg-primary/5 p-4"><p className="text-xs font-bold text-muted-foreground">Weighted KPI score</p><p className="mt-1 font-mono-ui text-3xl font-medium text-primary">{summary.total}</p></div></div>{mc ? <div className="mt-3 rounded-xl border border-primary/25 bg-primary/5 p-4" data-testid="panel-mc-summary"><div className="flex items-center justify-between"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-muted-foreground">Monte Carlo — best of {mc.iterations}</p><p className="font-mono-ui text-[10px] text-muted-foreground">seed {mc.seed}</p></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs font-bold text-muted-foreground">Best weighted KPI</p><p className="mt-1 font-mono-ui text-2xl font-medium text-primary">{pct(mc.best.kpi.weightedTotal)}</p></div><div><p className="text-xs font-bold text-muted-foreground">Best rank-1 hit rate</p><p className="mt-1 font-mono-ui text-2xl font-medium text-primary">{pct(mc.best.kpi.rank1HitRate)}</p></div><div><p className="text-xs font-bold text-muted-foreground">Weighted KPI range</p><p className="mt-1 font-mono-ui text-2xl font-medium text-primary">{pct(mc.distribution.weightedTotal.p10)}–{pct(mc.distribution.weightedTotal.p90)} <span className="text-sm text-muted-foreground">mean {pct(mc.distribution.weightedTotal.mean)}</span></p></div><div><p className="text-xs font-bold text-muted-foreground">Rank-1 hit range</p><p className="mt-1 font-mono-ui text-2xl font-medium text-primary">{pct(mc.distribution.rank1HitRate.p10)}–{pct(mc.distribution.rank1HitRate.p90)} <span className="text-sm text-muted-foreground">mean {pct(mc.distribution.rank1HitRate.mean)}</span></p></div></div><div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-border bg-card/60 p-4" data-testid="panel-hard-result"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-muted-foreground">Hard result</p><div className="mt-3 space-y-1.5 text-xs">{room.groups.map((group) => <div key={group.code} className="flex items-center justify-between"><span className="font-semibold text-foreground">{group.code}</span><span className="font-mono-ui text-muted-foreground">{group.participantIds.length} · {group.maleCount}M / {group.femaleCount}F · parity {pct(mc.metrics.groupParity[group.code] ?? 0)}</span></div>)}<div className="mt-2 flex items-center justify-between border-t border-border pt-2"><span className="font-semibold text-foreground">{mc.metrics.totalLeaders} of {room.participants.length} placed</span><span className="font-mono-ui">{mc.metrics.genderDistribution.males}M / {mc.metrics.genderDistribution.females}F</span></div><div className="flex items-center justify-between"><span className="font-semibold text-foreground">{mc.best.forcedCount} forced placement{ mc.best.forcedCount === 1 ? "" : "s"}</span><span className={`font-mono-ui ${room.allocationWarnings.length ? "text-accent-foreground" : "text-[#477d5b]"}`}>{room.allocationWarnings.length} warning{ room.allocationWarnings.length === 1 ? "" : "s"}</span></div></div></div><div className="rounded-xl border border-border bg-card/60 p-4" data-testid="panel-soft-kpi"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-muted-foreground">Soft KPI — best run</p><div className="mt-3 space-y-1.5">{softRows.map(([label, value]) => <div key={label} className="flex items-center justify-between text-xs"><span className="text-muted-foreground">{label}</span><span className="font-mono-ui font-semibold text-primary">{pct(value)}</span></div>)}</div></div></div></div> : null}</section>;
}
